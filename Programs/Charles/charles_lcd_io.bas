...
130 PRINT "=========================="
140 PRINT
170 PRINT "BUTTONS: 1 FEED, 2 PLAY, 4 PET, 8 ANNOY,";
180 PRINT " 64 DEBUG, 128 QUIT."
185 PRINT
...
370 LET IA = 0 : REM Ticks since the last physical action
...
680 GOSUB 4000
690 LET MC = MC + 1
700 IF MO <> PS THEN GOTO 730
710 IF MC < MI THEN GOTO 750
730 GOSUB 5000
740 LET MC = 0
750 GOSUB 6000
760 GOTO 600
...
1200 LET AD = AD + 1
1210 LET IA = IA + 1
1220 IF IA < 10 THEN GOTO 1250
1230 LET IA = 0 : LET LA = 0 : LET RP = 0
1250 IF AD < 10 THEN RETURN
1260 LET AD = 0
1270 IF NA > 0 THEN LET NA = NA - 1
1280 RETURN
...
5670 NEXT LC
5680 RETURN
5900 REM LCD delay split into four Digital IO sampling intervals
5910 FOR LQ = 1 TO 4
5920 LET PK = INP(IP)
...
6830 LET LA = AK
6840 LET IA = 0
6850 IF AK <> 4 THEN RETURN
6860 LET NA = NA + 1 : LET AD = 0
6870 IF NA > 9 THEN LET NA = 9
6880 RETURN
7000 REM Poll the Digital IO buttons and process one valid press
7010 LET BK = PB : LET PB = 0
7020 IF BK = 0 THEN LET BK = INP(IP)
7030 IF BK = 0 THEN RETURN
7040 IF BK = 1 OR BK = 2 OR BK = 4 OR BK = 8 THEN GOTO 7090
7050 IF BK = 64 OR BK = 128 THEN GOTO 7090
7060 PRINT "PRESS ONE RECOGNISED DIGITAL IO BUTTON."
7070 GOSUB 7300
7080 RETURN
7090 OUT IP, BK : REM Light the corresponding feedback LED
7100 IF BK = 64 THEN GOTO 7250
7110 IF BK = 128 THEN GOTO 7270
7120 LET OM = MO : LET AK = 0
7130 IF BK = 1 THEN LET AK = 1 : REM Feed
7140 IF BK = 2 THEN LET AK = 2 : REM Play
7150 IF BK = 4 THEN LET AK = 3 : REM Pet
7160 IF BK = 8 THEN LET AK = 4 : REM Annoy
7170 GOSUB 6800
7180 IF AK = 1 THEN GOSUB 7400
7190 IF AK = 2 THEN GOSUB 7600
7200 IF AK = 3 THEN GOSUB 7800
7210 IF AK = 4 THEN GOSUB 8200
7220 GOSUB 8400
7230 GOSUB 4000 : LET PS = MO : LET MC = 0
7240 GOSUB 2500 : GOSUB 7300 : RETURN
7250 GOSUB 2000
