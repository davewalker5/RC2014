...
class AssemblyTests(unittest.TestCase):
    ...
    def _check_execution(self, trace: str, mode: int) -> None:
        ...
        expected_speech = generate._speech_records(generate.CUES)
        ...
        # At the supplied settings, success ends at 3200 * 5 ms (including
        # the tail); a permanently busy card times out at 3800 * 5 ms.
        # These assertions do not claim an exact SP0256 acoustic model.
        self.assertEqual(int(rows[-1][2]), 1 if mode == 2 else 0)
        self.assertEqual(int(rows[-1][4]), 3800 if mode == 2 else 3200)
        # Allow bounded scheduler overhead above the nominal 16/19 seconds.
        ...
        self.assertGreater(elapsed, 19 if mode == 2 else 16)
        self.assertLess(elapsed, 19.5 if mode == 2 else 16.5)
